...
architecture MINIuP_T7Behavior of MINIuP_T7 is

    signal clk_base                           : std_ulogic;
    signal irclk, ipclk, accuclk, outclk      : std_ulogic;
    signal clk_cpu                            : std_ulogic;
...
begin
    ...
    dspsel <= SW(0);
    inport <= SW(15 downto 8);
   
    BCLK      : Aux_ClockDiv generic map ( divider => 12 ) -- 100e6MHz/12=8.33MHz
                         port map ( RSTN=>RESETN,
                                    INCLK=>CLK100MHz, OUTCLK=>clk_base );

    CPUCLK    : Aux_ClockDiv generic map ( divider => 5e6 ) -- 100e6MHz/12=8.33MHz
                         port map ( RSTN=>RESETN,
                                    INCLK=>CLK100MHz, OUTCLK=>clk_cpu );-- the generic map and the port map for the CPU clock should be placed here.
    
    CTRL      : CtrlUnitV3 port map ( CLK=>clk_cpu, RSTN=>RESETN,
                                      OPCODE=>opcodeo, ZERO=>zero, SIGN=>sign, STATEIND=>stind,
                                      IPCLK=>ipclk, IRCLK=>irclk, ACCLK=>accuclk, OUTCLK=>outclk,
                                      RE=>re, IPLOADN=>iploadn);
    
    IP        : InstrPtr port map ( IPCLK=>ipclk, RSTN=>RESETN,
                                    IPLOADN=>iploadn, DSTPTR=>operando, ISTPTR=>istptr);
                                    
    PRG       : ProgROM_Testprog4 port map ( ADDRESS=>istptr, OPCODE=>opcodei, OPERAND=>operandi);
    ...
    MICROPROG : MicroProgV3 port map ( OPCODE=>opcodeo, S=>s,
    ...
    BLOCKRAM  : RAM generic map ( addrWidth => 8, dataWidth => 8)
                    port map ( CLK=>clk_cpu, RE=>re, WE=>we, OE=>oe,
                               ADDR=>operando, DATAIN=>std_ulogic_vector(iobus),std_logic_vector(DATAOUT)=>iobus);

    INPUTPORT : InBuffer port map ( EN=>ipen, DIN=>inport, DOUT=>iobus );      
