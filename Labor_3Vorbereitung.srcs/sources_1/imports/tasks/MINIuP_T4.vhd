...
architecture MINIuP_T4Behavior of MINIuP_T4 is
    ...
    -- the component declaration for the control unit is missing here    
    component CtrlUnitV1 is
    ...
    Port ( CLK, RSTN                     : in  std_ulogic;
           ...
           BTND, BTNU                    : in  std_ulogic;
           IRCLK, ACCUCLK, OUTCLK        : out std_ulogic;
           IPCLK, MANCLK                 : out std_ulogic); 
    end component;
...
begin
    ...
    CTRL      : CtrlUnitV1 port map ( CLK=>manclk, RSTN=>RESETN,
                                      OPCODE=>opcodeo, STATEIND=>stind,
                                      IPCLK=>ipclk, IRCLK=>irclk, ACCLK=>accuclk, OUTCLK=>outclk); -- the port map for the control unit is missing here
    
    IP        : InstrPtr port map ( IPCLK=>ipclk, RSTN=>RESETN,
                                    IPLOADN=>'1', DSTPTR=>(others => '-'), ISTPTR=>istptr);
                                    
    PRG       : ProgROM_Testprog1 port map ( ADDRESS=>istptr, OPCODE=>opcodei, OPERAND=>operandi);
    ...
    LED(13)          <= carry;
    LED(12)          <= overflow;
    -- the assignement for the LEDs LD10..9 for the status indicator is missing here
	LED(10 downto 9) <= stind;
	
	
    LED(7 downto 0)  <= outsig;
                                              
end MINIuP_T4Behavior;
