...
architecture MINIuP_T6Behavior of MINIuP_T6 is
...
begin
    ...
    CTRL      : CtrlUnitV3 port map ( CLK=>clk_base, RSTN=>RESETN,
                                      OPCODE=>opcodeo, ZERO=>zero, SIGN=>sign, STATEIND=>stind,
                                      IPCLK=>ipclk, IRCLK=>irclk, ACCLK=>accuclk, OUTCLK=>outclk,
                                      RE=>re, IPLOADN=>iploadn); -- please add the port map for the control unit here
    
    IP        : InstrPtr port map ( IPCLK=>ipclk, RSTN=>RESETN,
                                    IPLOADN=>iploadn, DSTPTR=>operando, ISTPTR=>istptr);
                                    
    PRG       : ProgROM_Testprog3 port map ( ADDRESS=>istptr, OPCODE=>opcodei, OPERAND=>operandi);
